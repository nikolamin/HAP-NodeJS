...
Characteristic.prototype.updateValue = function(newValueRaw, callback, context) {

  if ( newValueRaw instanceof Error ) {
    this.status = newValueRaw
  } else {
    this.status = null;
  }

  let newValue = this.validateValue(newValueRaw); //validateValue returns a value that has be cooerced into a valid value.
